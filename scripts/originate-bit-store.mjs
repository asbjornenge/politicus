...
const artifactPath = join(repoRoot, 'artifacts/BitDataStore.json');
if (!existsSync(artifactPath)) {
  console.error('artifacts/BitDataStore.json missing. Run `npm run compile` first.');
  process.exit(1);
}
...
if (net.BitDataStore) {
  console.log(`BitDataStore already deployed at ${net.BitDataStore}.`);
  process.exit(0);
}
...
console.log(`Originating BitDataStore to ${rpcUrl}`);
console.log(`  bootstrap admin = ${POLITICUS_ADDRESS} (will transfer to BitRegistryLogic after that's deployed)`);
...
deployments[network] = { ...(deployments[network] ?? {}), BitDataStore: addr };
writeFileSync(deploymentsPath, JSON.stringify(deployments, null, 2) + '\n');
console.log(`Recorded in deployments.json under "${network}"`);
console.log(`\nNext: npm run originate:bit-logic`);
